...
Renderer::Renderer(const char *image, Vector2 size): m_size(size), m_img_path(image) {
    ...
    m_texture = SDL_CreateTextureFromSurface(GameRenderer::get_main_renderer(), m_surface);
...
void Renderer::update() {
    SDL_RenderCopy(GameRenderer::get_main_renderer(), m_texture, nullptr, m_position);
}
